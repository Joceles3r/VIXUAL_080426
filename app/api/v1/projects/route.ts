 ...
 *
 * Collection endpoints only (Next.js route handlers must be named exports
 * matching HTTP verbs at this path level):
 * GET  /api/v1/projects  — List projects for authenticated user
 * POST /api/v1/projects  — Create a new project draft
 *
 * Per-resource operations (GET/PUT/DELETE/submit/approve/reject) are handled
 * in dedicated sub-routes: app/api/v1/projects/[id]/route.ts etc.
 */
...
export async function POST(req: NextRequest) {
  try {
    ...
    // Only users with a creator role can create projects
    if (!ctx.userRoles.some((r) => ["creator", "infoporteur", "podcasteur"].includes(r))) {

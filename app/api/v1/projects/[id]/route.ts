...
export async function GET(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    const isAdmin = ctx.userRoles.includes("admin") || ctx.userRoles.includes("patron")
...
export async function PUT(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    if (error.code === "PROJECT_NOT_FOUND") return errorResponse("Project not found", 404)
    if (error.code === "ACCESS_DENIED") return errorResponse("Access denied", 403)
    if (error.code === "INVALID_TRANSITION") return errorResponse(error.message, 400)
...
export async function DELETE(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    if (error.code === "PROJECT_NOT_FOUND") return errorResponse("Project not found", 404)
    if (error.code === "ACCESS_DENIED") return errorResponse("Access denied", 403)
    if (error.code === "CANNOT_DELETE") return errorResponse(error.message, 400)
